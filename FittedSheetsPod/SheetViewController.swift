...
open class SheetViewController: UIViewController {
    ...
    public enum Stretchability {

        case resistant
        case fixedDistance(CGFloat)
        case none
    }
    ...
    /// How does it behave when user tries to drag it beyond its max height
    public var stretchability: Stretchability = .resistant
    ...
    @objc func panned(_ gesture: UIPanGestureRecognizer) {
        ...
        if newHeight > maxHeight {
            switch stretchability {
            case let .fixedDistance(distance):
                newHeight = maxHeight + min(newHeight - maxHeight, distance)
            case .resistant:
                newHeight = maxHeight + sqrt(newHeight - maxHeight)
            case .none:
                newHeight = maxHeight
            }
        }

        print("panOffsetY: \(panOffsetY) newHeight: \(newHeight)")
